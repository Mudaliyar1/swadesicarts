...
function scanObject(obj, isUrlQuery = false, req = null) {
    ...
    for (const key in obj) {
        ...
        if (typeof value === 'object') {
            const error = scanObject(value, isUrlQuery, req);
            ...
            const isPolicyPath = req && req.originalUrl && req.originalUrl.includes('/admin/policies');
            const isContentField = lowerKey === 'content';

            if (!isContentField && !isPolicyPath) {
...
const securityValidation = (req, res, next) => {
    ...
    for (const source of sources) {
        const errorMsg = scanObject(source.data, source.name === 'query', req);
        if (errorMsg) {
