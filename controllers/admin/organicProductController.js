...
exports.list = async (req, res) => {
  try {
    ...
    const totalProducts = await OrganicProduct.countDocuments(filter);
    const products = await OrganicProduct.find(filter)
      .sort(sort)
      .skip(skip)
      .limit(limit);

    // Get unique categories for the filter dropdown
    const categories = await OrganicProduct.distinct('category');
...
exports.bulkDelete = async (req, res) => {
  try {
    ...
    const products = await OrganicProduct.find({ _id: { $in: ids } });

    // Delete from DB first for instant user response
    await OrganicProduct.deleteMany({ _id: { $in: ids } });
